...
export function PublicGuestPreview({ guests }: { guests: PublicGuest[] }) {
  return (
    <section className="border-t border-zinc-950/10 pt-6">
      <h2 className="text-2xl font-black lowercase text-zinc-950">Guests</h2>
      {guests.length ? (
        <div className="mt-4 flex flex-wrap gap-2">
          {guests.map((guest, index) => (
            <div
              key={guest.id}
              className="animate-float flex items-center gap-2 rounded-full bg-white/34 px-2.5 py-2 backdrop-blur"
