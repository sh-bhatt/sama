  ...
  return (
    <section className="border-t border-zinc-950/10 pt-6">
      <div className="flex items-end justify-between gap-4">
        <div>
          <h2 className="text-2xl font-black lowercase text-zinc-950">Updates</h2>
        </div>
      </div>
        ...
        {broadcasts.map((broadcast) => (
          ...
          >
            <div className="flex flex-wrap items-center gap-2">
              {broadcast.pinned && (
                <span className="rounded-full bg-white/72 px-3 py-1 text-xs font-black text-lime-mute">
                  pinned
                </span>
              )}
              <span className={broadcast.pinned ? "text-xs font-black text-zinc-800" : "text-xs font-black text-zinc-600"}>
                {formatEventDateShort(broadcast.createdAt)}
              </span>
            </div>
            <h2 className="mt-2 text-xl font-black">{broadcast.title}</h2>
            <p className={broadcast.pinned ? "mt-2 font-semibold text-zinc-900" : "mt-2 font-semibold leading-7 text-zinc-700"}>
              {broadcast.message}
            </p>
          </article>
        ))}
