      ...
      <section className="mx-auto grid max-w-7xl gap-6 px-4 py-5 sm:px-6 lg:grid-cols-[minmax(0,1fr)_24rem] lg:px-8 lg:py-8">
        ...
        <aside className="min-w-0 space-y-4 lg:sticky lg:top-5 lg:self-start">
          <section className="rounded-[2rem] border border-white/10 bg-zinc-950 p-5 shadow-[0_22px_80px_rgba(0,0,0,0.5)]">
            ...
            <div className="mt-5 grid gap-2">
              {["Going", "Maybe", "Not going"].map((item, index) => (
                <button
                  ...
                  disabled
                  className={[
                    "rounded-2xl px-4 py-4 text-left font-black opacity-90",
                    index === 0 ? "bg-ivory text-zinc-950" : "bg-white/8 text-white",
                  ].join(" ")}
                >
                  {item}
                </button>
            ...
            <p className="mt-4 rounded-2xl bg-white/8 px-4 py-3 text-sm font-bold text-zinc-300">
              RSVPs connect in the next phase.
            </p>
          </section>
          ...
          <section className="theme-panel rounded-[2rem] border p-5">
            <p className="mb-4 text-sm font-black uppercase tracking-[0.18em] text-rose-neon">activity preview</p>
      ...
      <div className="fixed inset-x-3 bottom-3 z-40 rounded-full border border-white/10 bg-zinc-950/92 p-2 shadow-[0_20px_70px_rgba(0,0,0,0.55)] backdrop-blur lg:hidden">
        <div className="grid grid-cols-3 gap-2">
          {["Going", "Maybe", "Share"].map((item, index) => (
            <button key={item} type="button" disabled className={index === 0 ? "rounded-full bg-ivory px-3 py-3 text-sm font-black text-zinc-950" : "rounded-full bg-white/8 px-3 py-3 text-sm font-black text-white"}>
              {item}
            </button>
          ))}
        </div>
